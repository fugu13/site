...
/// The path pages should link to.
///
/// Falls back to the unhashed `/style.css` if never set, so a page rendered
/// outside of `prerender` (e.g. directly in a unit test) still has a sane
/// value rather than panicking.
pub fn stylesheet_path() -> &'static str {
    STYLESHEET_PATH.get().map_or("/style.css", String::as_str)
}
