...
Alongside the pages, the build writes three machine-readable files to `dist/`'s top level — `sitemap.xml`, `atom.xml`, and `robots.txt` — generated from the same parsed posts the pages render from, so they can never drift from the routes that exist. `docs/seo.md` describes them.
...
timeline
    ...
    Machine outputs : Write sitemap.xml, atom.xml, and robots.txt from the same parsed posts
